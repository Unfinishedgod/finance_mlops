...
# Google Storage 적재
source_file_name = f'data_crawler/cleaning/kor_stock_ohlcv/buy_sell_count_kospi.parquet'    # GCP에 업로드할 파일 절대경로
destination_blob_name = f'data_crawler/cleaning/kor_stock_ohlcv/buy_sell_count_kospi.parquet'    # 업로드할 파일을 GCP에 저장할 때의 이름
bucket = storage_client.bucket(bucket_name)
blob = bucket.blob(destination_blob_name)
blob.upload_from_filename(source_file_name)
...
os.system(f"/home/shjj08choi4/anaconda3/bin/python3 python_file/send_gmail.py '코스피_전처리'")
